...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'pts_voxel_encoder': dict(lr_mult=0.3),
            'pts_middle_encoder': dict(lr_mult=0.3),
            'pts_backbone': dict(lr_mult=0.3),
            'pts_neck': dict(lr_mult=0.3),
            'img_backbone': dict(lr_mult=0.5),
            'img_neck': dict(lr_mult=0.5),
            'pts_bbox_head': dict(lr_mult=2.0),  
        }
    )             
...
momentum_config = None  # 用 cosine 时去掉 cyclic momentum
total_epochs = 30
...
log_level = 'INFO'
work_dir = None
load_from = '/wangjinhai1/zYu/QTNet/work_dirs/transfusionLC_stage1/epoch_10.pth'
# load_from = None
resume_from = None
workflow = [('train', 1)]

...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        num_proposals=64, auxiliary=True, in_channels=256*2, hidden_channel=128,
        ...
        loss_vis=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.1),
        loss_cause=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.1),
    ),
    train_cfg=dict(pts=dict(
        ...
        pos_weight=-1, gaussian_overlap=0.3, min_radius=3,
        grid_size=[
            int((point_cloud_range[3]-point_cloud_range[0])/voxel_size[0]),
            int((point_cloud_range[4]-point_cloud_range[1])/voxel_size[1]),

# ====================== MineGrape - LC Stage-2 (unfreeze LiDAR) ======================
# 完整参数与 Stage-1 相同，仅训练策略不同（解冻 pts_*，从 Stage-1 权重继续）
...
model = dict(
    ...
    pts_middle_encoder=dict(
        type='SparseEncoder', in_channels=4, sparse_shape=[41,400,400], output_channels=128,
        order=('conv','norm','act'), encoder_channels=((16,16,32),(32,32,64),(64,64,128),(128,128)),
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(type='TransFusionBBoxCoder', pc_range=point_cloud_range[:2], voxel_size=voxel_size[:2],
...
# ---------------------- Train (解冻 LiDAR，全量联合) ----------------------
optimizer = dict(
    type='AdamW', lr=5e-5, weight_decay=0.01,  # 二阶段 lr 略小
    paramwise_cfg=dict(custom_keys={
        'img_backbone': dict(lr_mult=0.5),  # 仍稍小
...
log_level = 'INFO'
work_dir = None
load_from = '/path/to/stage1_checkpoint.pth'  # ← 放 Stage-1 的权重
resume_from = None

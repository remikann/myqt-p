...
@PIPELINES.register_module()
class LoadMultiFeaturesFromFile(object):
    def __init__(self,
                 num_frames=1,
                 file_client_args=dict(backend='disk')):
    ...
    def _load_feature(self, features_filename):
        feature = np.load(features_filename)
        return feature

    def _load_pred_results(self, pred_results_filename):
        pred_result = pickle.load(open(pred_results_filename, 'rb'))
        return pred_result

    def __call__(self, results):
        features_filename = results['features_filename']
        pred_results_filename = results['pred_results_filename']
        queries = self._load_feature(features_filename)
        pred_results = self._load_pred_results(pred_results_filename)
